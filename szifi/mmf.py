...
class cluster_finder:
    ...
    rank=0):
        ...
        self.profile_params = {}
        if self.params_model['profile_type']=='arnaud':
            self.profile_params['theta_500_vec_arcmin'] = self.params_szifi["theta_500_vec_arcmin"]
        ...
        if self.params_szifi['mmf_type']!='standard' and self.params_model['profile_type']!='arnaud':
            raise Exception("Non-standard MMF not supported for non-SZ shapes!")

        if self.params_szifi["mmf_type"] == "standard":
            
            if self.params_model['profile_type']=='arnaud':
                print("Using a tSZ SED")
                a_matrix = np.zeros((len(self.exp.tsz_sed),1))
                a_matrix[:,0] = self.exp.tsz_sed
                self.params_szifi["a_matrix"] = a_matrix
            
            elif self.params_model['profile_type']=='point':
                print("Using a point-source SED")
                a_matrix = np.zeros((len(self.exp.png_sed),1))
                a_matrix[:,0] = self.exp.png_sed
                self.params_szifi["a_matrix"] = a_matrix
            
            elif self.params_model['profile_type']=='soubhik':
                print("Using a primordial-feature SED")
                a_matrix = np.zeros((len(self.exp.png_sed),1))
                a_matrix[:,0] = self.exp.png_sed
                self.params_szifi["a_matrix"] = a_matrix
        ...
        if self.params_szifi["deproject_cib"] is not None:
            if self.params_model['profile_type']!='arnaud':
                raise Exception("CIB deprojection not supported for non-SZ shapes!")
    ...
    def find_clusters(self):
        ...
        for field_id in self.data_file["params_data"]["field_ids"]:
            ...
            self.results.profile_params = self.profile_params
            ...
            clusters_masked_old = cat.cluster_catalogue(self.params_model['profile_type'])
            ...
            while i >= 0:
                ...
                if self.params_szifi["iterative"] == False:
                    ...
                    assert self.params_model['profile_type'] == 'arnaud', "Not yet implemented!"
...
class filter_maps:
    ...
    def __init__(self,t_obs=None,inv_cov=None,pix=None,cosmology=None,profile_params=None,
    ...
    def find_clusters(self):
        ...
        if self.params_model['profile_type']=='arnaud':
            n_theta = len(self.profile_params['theta_500_vec_arcmin'])
        elif self.params_model['profile_type']=='point':
            n_theta = 1
        elif self.params_model['profile_type']=='soubhik':
            n_theta = len(self.profile_params['eta_star_vec'])
            assert n_theta==len(self.profile_params['eta_hs_vec'])
        ...
        for k in range(0,n_subgrid):
            ...
            for j in range(0,n_theta):
                ...
                if self.params_model["profile_type"] == "point":
                    ...
                    if self.rank == 0: print("Theta",j,self.profile_params['theta_500_vec_arcmin'][j])

                    z = 0.2
                    M_500 = model.get_m_500(self.profile_params['theta_500_vec_arcmin'][j],z,self.cosmology)
                    nfw = model.gnfw(M_500,z,self.cosmology,
                    type=self.params_model["profile_type"])

                    t_tem,t_tem_nc = nfw.get_t_map_convolved(self.pix, self.exp,beam=self.params["beam"],theta_cart=[(0.5*self.pix.nx+subgrid_ix[k])*self.pix.dx, (0.5*self.pix.nx+subgrid_jx[k])*self.pix.dx],get_nc=True,sed=False)
                    ...
                    if self.rank == 0: print("eta_*",j,self.profile_params['eta_star_vec'][j])
        ...
        for i in range(0,n_mask_select):
            ...
            cat_new = cat.cluster_catalogue(self.params_model['profile_type'])

            cat_new.catalogue["q_opt"] = q_opt
            cat_new.catalogue["y0"] = y0_est
            if self.params_model["profile_type"] == "arnaud":
                cat_new.catalogue['theta_500'] = self.profile_params['theta_500_vec_arcmin'][indices[2]]
            elif self.params_model["profile_type"] == "soubhik":
                cat_new.catalogue["eta_star"] = self.profile_params['eta_star_vec'][indices[2]]
                cat_new.catalogue["eta_hs"] = self.profile_params['eta_hs_vec'][indices[2]]
    ...
    def extract_at_true_values(self,t_true,true_catalogue):
        ...
        catalogue_at_true_values = cat.cluster_catalogue(self.params_model['profile_type'])
