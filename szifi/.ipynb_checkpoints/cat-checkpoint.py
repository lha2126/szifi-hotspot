...
class cluster_catalogue:
    ...
    def __init__(self, profile_type='arnaud'):
    ...
    def initialise(self):
        ...
        if self.profile_type=='arnaud':
            self.catalogue["theta_500"] = np.empty(0)
            self.catalogue["m_500"] = np.empty(0)
        elif self.profile_type=='soubhik':
            self.catalogue['eta_star'] = np.empty(0)
...
mode="close+highest_q",sort=True,unique=False,verb=True):
    ...
    for i in range(0,n_clusters_1):
        ...
        if verb: print(i,"of",n_clusters_1)
...
class detection_processor:
    ...
    def __init__(self,results_dict,params_szifi,profile_type):
        ...
        if profile_type=='arnaud':
            n_theta = len(params_szifi["theta_500_vec_arcmin"])
        elif profile_type=='point':
            n_theta = 1
        elif profile_type=='soubhik':
            n_theta = len(params_szifi["eta_star_vec"])
        ...
        if self.profile_type=='arnaud':
            ...
            self.results.eta_star_vec = params_szifi['eta_star_vec']
