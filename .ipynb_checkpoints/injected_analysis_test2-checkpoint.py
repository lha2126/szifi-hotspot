...
if g==0:
    if os.path.exists('outputs_test2_v3/ffp10_%d_batch%d_%s.npy'%(sim_no,batch_no,analysis_type)):
        print("Output already computed; exiting!")
        sys.exit()
else:
    if os.path.exists('outputs_test2_v3/ffp10_%d_batch%d_%s_inj-g%.2f.npy'%(sim_no,batch_no,analysis_type,g)):
        print("Output already computed; exiting!")
        sys.exit()
...
for i in params_data['field_ids']:
    if not os.path.exists(cutout_dir+"planck_field_" + str(i) + "_inj_pair.npy"): all_exist=False
...
if all_exist:
    ...
    # Load map
    injection_map = hp.read_map('/insomnia001/home/lha2126/ceph/szifi_cutouts/cutout300_pairs_sim%d.fits'%sim_no, field=[0,1,2,3,4,5])
    ...
    for i in params_data['field_ids']:
        ...
        np.save(cutout_dir+"planck_field_" + str(i) + "_inj_pair.npy",inj_output)
...
for f in params_data['field_ids']:
    data.data['t_obs'][f] += g*np.load(cutout_dir+"planck_field_" + str(f) + "_inj_pair.npy")[0]
...
if g==0:
    np.save('outputs_test2_v3/ffp10_%d_batch%d_%s.npy'%(sim_no,batch_no,analysis_type),catalogue)
else:
    np.save('outputs_test2_v3/ffp10_%d_batch%d_%s_inj-g%.2f.npy'%(sim_no,batch_no,analysis_type,g),catalogue)    