...
if os.path.exists('outputs_planck/planck_batch%d_%s.npy'%(batch_no,analysis_type)):
...
if analysis_type =='point':
    cutout_dir = '/insomnia001/home/lha2126/ceph/szifi_cutouts/planck_raw/'
else:
    cutout_dir = '/insomnia001/home/lha2126/ceph/szifi_cutouts/planck_cleaned/'
...
if analysis_type=='point':
    ...
    for freq in range(6):
        ...
        q_th_final = 4.
        try:
            point_catalogue = szifi.get_catalogue_q_th(point_catalogue,q_th_final)
        except TypeError:
            point_catalogue = szifi.cat.cluster_catalogue()
            
        point_catalogues.append(point_catalogue)

    np.save('outputs_planck/planck_batch%d_%s.npy'%(batch_no,analysis_type),point_catalogues)
    ...
    np.save('outputs_planck/planck_batch%d_%s.npy'%(batch_no,analysis_type),catalogue)